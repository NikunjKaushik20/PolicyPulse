"""
Test queries for NREGA and RTI policies.
...
# Demo queries with expected beautiful outputs
DEMO_QUERIES = {
    "NREGA": [
...
# --- Multimodal and Cross-Modal Test Cases ---
def test_audio_query():
    from .reasoning import generate_reasoning_trace
    trace = generate_reasoning_trace(
        query="What urgency is present in the 2023 NREGA speech?",
        policy_id="NREGA",
        top_k=3
    )
    assert any(pt.get("modality") == "audio" for pt in trace.get("retrieved_points", []))
    assert any(pt.get("audio_timestamp") is not None for pt in trace.get("retrieved_points", []))

def test_video_query():
    from .reasoning import generate_reasoning_trace
    trace = generate_reasoning_trace(
        query="Show public safety anomalies in 2022 Smart Cities video footage.",
        policy_id="SMART-CITIES",
        top_k=3
    )
    assert any(pt.get("modality") == "video" for pt in trace.get("retrieved_points", []))
    assert any(pt.get("video_frame") is not None for pt in trace.get("retrieved_points", []))

def test_agentic_outputs():
    from .reasoning import generate_reasoning_trace
    trace = generate_reasoning_trace(
        query="Summarize the latest RTI amendment for a layperson.",
        policy_id="RTI",
        top_k=3
    )
    assert "layman_summary" in trace
    assert "advocacy_letter" in trace
